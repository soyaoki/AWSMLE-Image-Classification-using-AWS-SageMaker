...
import smdebug.pytorch as smd # Line added from hpo.py

logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)
logger.addHandler(logging.StreamHandler(sys.stdout))

def test(model, test_loader, criterion, device, hook): # Arg 'hook' added from hpo.py
    '''
    TODO: Complete this function that can take a model and a 
          testing data loader and will get the test accuray/loss of the model
          Remember to include any debugging/profiling hooks that you might need
    '''
    hook.set_mode(smd.modes.EVAL) # Line added from hpo.py
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def train(model, train_loader, criterion, optimizer, epochs, device, hook): # Arg 'hook' added from hpo.py
    '''
    TODO: Complete this function that can take a model and
          data loaders for training and will get train the model
          Remember to include any debugging/profiling hooks that you might need
    '''
    hook.set_mode(smd.modes.TRAIN) # Line added from hpo.py
...
def main(args):
    ...
    model = train(model, train_loader, loss_criterion, optimizer, args.epochs, device, hook) # Arg 'hook' added from hpo.py
    
    '''
    TODO: Test the model to see its accuracy
    '''
    logger.info("Testing the model.")
    test(model, test_loader, loss_criterion, device, hook) # Arg 'hook' added from hpo.py
